...
typedef struct {
  ...
  uint8_t   AccelScale;           // Acceleration scale
  uint16_t  GyroScale;            // Gyroscope scale
  float     Accel[3];             // Acceleration
  float     Gyro[3];              // Gyroscope
	uint8_t		ID[4];								// Sensor HW ID
	uint8_t		Error;								// if Sensor read or write fails
} _MPU6050_SENSOR __attribute__ ((aligned (4)));

extern void MPU6050_Init(uint8_t port);
extern void MPU6050_Read(void);
