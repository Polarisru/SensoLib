...
_MFRC522_SENSOR MFRC522_Sensor;
uint8_t MFRC522_Buffer[16];

/*
  Example from http://stm32f4-discovery.com/2014/07/library-23-read-rfid-tag-mfrc522-stm32f4xx-devices/
  ...
  More links:
  https://github.com/kbiva/stm32f103_libs
...
{
	MFRC522_CS_LOW;
  MFRC522_Buffer[0] = (addr << 1) & 0x7E;
  MFRC522_Buffer[1] = val;
  SPI_WriteData(MFRC522_Sensor.Port, MFRC522_Buffer, 2);
	MFRC522_CS_HIGH;
}
...
uint8_t MFRC522_ReadRegister(uint8_t addr) 
{
	...
	MFRC522_CS_LOW;
  MFRC522_Buffer[0] = ((addr << 1) & 0x7E) | 0x80;
  SPI_WriteData(MFRC522_Sensor.Port, MFRC522_Buffer, 1);
  SPI_ReadData(MFRC522_Sensor.Port, &val, 1);
	MFRC522_CS_HIGH;
...
// MFRC522_Write - 
//------------------------------------------------------------------------------
MFRC522_Status_t TM_MFRC522_Write(uint8_t blockAddr, uint8_t* writeData) 
{
	MFRC522_Status_t status;
	uint16_t recvBits;
	uint8_t i;
...
//------------------------------------------------------------------------------
// MFRC522_Init - initialize MFRC522, setup pins
//------------------------------------------------------------------------------
void MFRC522_Init(uint8_t Port)
{
  GPIO_InitTypeDef GPIO_InitStructure;
  
  if (Port>=SPI_NUM_LAST)
    return;
 
  MFRC522_Sensor.Port = Port;  
  ...
  GPIO_Init(MFRC522_CS_PORT, &GPIO_InitStructure); 
